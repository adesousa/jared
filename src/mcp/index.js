...
class MCPManager {
  ...
  async initialize() {
    for (const [name, serverConfig] of Object.entries(
      ...
      try {
        ...
        } else if (serverConfig.url) { if (serverConfig.transport === "streamable") { transport = new StreamableHTTPClientTransport(new URL(serverConfig.url), { requestInit: { headers: serverConfig.headers } }); } else { transport = new SSEClientTransport(new URL(serverConfig.url), { headers: serverConfig.headers }); } }
        if (transport) {
      ...
      } catch (err) { console.error(`Failed to load MCP server ${name}:`, err); }
    }
  }
  getTools() {
  ...
  hasTool(name) { return this.tools.some(t => t.function.name === name); }
