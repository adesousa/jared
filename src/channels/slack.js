...
class SlackChannel {
  ...
  async start() {
    ...
    bus.on("message:send", async payload => { if (payload.channel === "slack" && payload.meta?.slackChannel) { try { await this.web.chat.postMessage({ channel: payload.meta.slackChannel, text: payload.content }); } catch (e) { logger.error("Slack send error:", e); } }
    });
