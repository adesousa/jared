...
class DiscordChannel {
  ...
  start() {
    ...
    this.client.once("ready", () => { logger.info(`Discord channel connected as ${this.client.user.tag}`); });
    ...
    bus.on("message:send", async payload => {
      if (payload.channel === "discord" && payload.meta?.channelId) {
        try {
          ...
          if (channel) { await channel.send(payload.content); }
        } catch (e) { logger.error("Discord send error:", e); }
      }
    });
    this.client
      .login(this.token)
