export default {
  ...
  execute: async ({ task, label, role }, { agentManager, modelOverride, channel, userId, sessionId, bus }) => {
    ...
    agentManager.spinUp(task, { modelOverride, channel, userId, sessionId, isSubagent: true, role }).then(result => {
      ...
      bus.emit("message:send", { channel, userId, sessionId, content: `🔄 Background task "${displayLabel}" completed by \`${roleStr}\`:\n${result.content}` });
    }).catch(err => {
      bus.emit("subagent:end", { channel, sessionId });
      bus.emit("message:send", { channel, userId, sessionId, content: `❌ Background task "${displayLabel}" by \`${roleStr}\` failed: ${err.message}` });
    });
