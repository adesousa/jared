  ...
  async validate(command) {
    for (const pattern of this.blockedPatterns) {
      if (pattern.test(command)) {
        return { allowed: false, reason: `🚫 Blocked: dangerous pattern (${pattern})` };
      }
    }
    if (this.restrictToWorkspace && this.workspaceDir) { const wsCheck = this._checkWorkspaceRestriction(command); if (!wsCheck.allowed) return wsCheck; }
    ...
    if (this.mode === "allowlist" || this.mode === "confirm") {
      ...
      for (const segment of segments) {
        ...
        if (bin && !this.allowedBins.includes(bin)) {
          return { allowed: false, reason: `🚫 Blocked: "${bin}" not in allowed list` };
        }
      }
    }
    ...
    if (absPathMatch) {
      for (const match of absPathMatch) {
        ...
        if (!isSafe && !absPath.startsWith(this.workspaceDir)) {
          return { allowed: false, reason: `🚫 Blocked: "${absPath}" outside workspace (${this.workspaceDir})` };
        }
      }
    }
    return { allowed: true };
  ...
  _askUserConfirmation(command) {
    return new Promise(resolve => {
      ...
      if (!payload.handled) {
        ...
        rl.question(payload.promptText, answer => { rl.close(); payload.callback(answer); });
      }
    });
  }
}
